...
execute as @e[type=marker,tag=tilelocked,tag=player2d,tag=currentPlayer] at @s unless entity @e[type=marker,tag=unlocked,distance=-0..] if loaded ~ ~ ~ run function tilelocked:tile_init/init_tile
...
execute if score @s TileLockedData matches ..0 at @e[type=marker,tag=tilelocked,tag=player2d,tag=currentPlayer] unless entity @e[type=marker,tag=unlocked,distance=..0.71] if entity @e[type=marker,tag=unlocked,distance=..3] at @s as @e[tag=tilelocked,tag=prevPosMarker,sort=nearest,limit=1] at @s run function tilelocked:tile_init/teleport_player_back
# if no available tiles, player further than 3 blocks from last good position teleport to middle of nearest tile
execute if score @s TileLockedData matches ..0 at @e[type=marker,tag=tilelocked,tag=player2d,tag=currentPlayer] unless entity @e[type=marker,tag=unlocked,distance=..0.71] unless entity @e[type=marker,tag=unlocked,distance=..3] at @s positioned ~ ~-1 ~ as @e[type=block_display,tag=tilelocked,distance=0..,sort=nearest,limit=1] at @s run teleport @a[tag=currentPlayer] ~ ~1 ~
# if has available tile, unlock the new tile
execute if score @s TileLockedData matches 1.. as @e[type=marker,tag=tilelocked,tag=player2d,tag=currentPlayer] at @s unless entity @e[type=marker,tag=unlocked,distance=..0.71] if loaded ~ ~ ~ run function tilelocked:tile_init/init_tile
